...
const PADROES_TECNOLOGIA = {
  ...
  'hono-api': {
    dependencias: ['hono', '@hono/node-server', '@hono/zod-validator'],
    imports: ['hono', 'hono/cors', 'hono/logger'],
    exports: ['app'],
    estrutura: ['src/routes', 'src/middleware'],
    configs: ['src/index.ts', 'src/index.js'],
    scripts: ['dev', 'start'],
    codigoPatterns: [/new\s+Hono\(\)/i, /app\.(get|post|put|delete|patch|use)\(/i, /c\.(json|text|html|redirect)/i, /return\s+c\.json/i],
    antiPatterns: []
  },
  'prisma-orm': {
    ...
    exports: ['prisma'],
    estrutura: ['prisma', 'prisma/schema.prisma'],
    configs: ['schema.prisma'],
    scripts: ['prisma:generate', 'prisma:migrate'],
    codigoPatterns: [/new\s+PrismaClient\(\)/i, /prisma\.\w+\.(findMany|findUnique|create|update|delete)/i],
    antiPatterns: []
  },
  'trpc-api': {
    dependencias: ['@trpc/server', '@trpc/client', '@trpc/react-query'],
    imports: ['@trpc/server', '@trpc/client'],
    exports: ['appRouter', 't'],
    estrutura: ['src/server/trpc', 'src/trpc'],
    configs: [],
    scripts: [],
    codigoPatterns: [/initTRPC\.create\(\)/i, /t\.router\(\{/i, /t\.procedure/i, /publicProcedure/i],
    antiPatterns: []
  },
  'zod-validation': {
    dependencias: ['zod'],
    ...
    configs: [],
    scripts: [],
    codigoPatterns: [/z\.object\(\{/i, /z\.string\(\)/i, /z\.infer<typeof/i, /\.parse\(/i, /\.safeParse\(/i],
    antiPatterns: []
  },
  'vitest-testing': {
    dependencias: ['vitest', '@vitest/ui', '@vitest/coverage-v8'],
    imports: ['vitest'],
    exports: [],
    estrutura: ['tests', '__tests__', 'src/__tests__'],
    configs: ['vitest.config.ts', 'vitest.config.js'],
    scripts: ['test', 'test:ui', 'coverage'],
    codigoPatterns: [/describe\(/i, /it\(/i, /test\(/i, /expect\(/i, /vi\.fn\(\)/i, /vi\.mock\(/i],
    antiPatterns: []
  }
};
