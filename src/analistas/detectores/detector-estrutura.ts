...
export const detectorEstrutura = {
  ...
  aplicar(_src: string, _relPath: string, _ast: NodePath<Node> | null, _fullPath?: string, contexto?: ContextoExecucao): TecnicaAplicarResultado {
    ...
    if (expectedDirs.length > 0) {
      const topLevelDirs = new Set(caminhosNorm
        .filter(p => p.includes('/') && !p.startsWith('node_modules/') && !p.startsWith('dist/'))
        .map(p => p.split('/')[0])
      );
      const configuredNames = new Set(
        Object.values(getNameConventions())
          .filter((v): v is string => typeof v === 'string' && v.trim().length > 0)
          .map(v => v.trim().toLowerCase())
      );
      for (const { key, expected } of expectedDirs) {
        const expectedLower = expected.toLowerCase();
        const match = [...topLevelDirs].find(d => d.toLowerCase() === expectedLower);
        if (!match) {
          const variantPattern = getDirVariantPattern(key);
          const foundVariant = [...topLevelDirs].find(d => variantPattern.test(d));
