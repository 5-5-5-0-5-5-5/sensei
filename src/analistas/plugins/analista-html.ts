...
function collectHtmlIssuesAst(src: string, relPath: string): Msg[] {
  ...
  const startLineOf = (n: AnyNode) => {
    const i = typeof (n as unknown as {
      startIndex?: number | null;
    }).startIndex === 'number' ? (n as unknown as {
      startIndex?: number | null;
  ...
  walk(doc as unknown as AnyNode, n => {
    if (n.type === 'directive') {
      const data = String((n as unknown as {
        data?: string;
      }).data || '');
    ...
    if (tag === 'meta' && !isTemplate) {
      ...
      if (httpEquiv === 'content-type' && /charset\s*=\s*utf-8/i.test(content)) hasCharset = true;
    ...
    // Validação de botões
    if (tag === 'button') {
      const text = (n as NodeWithChildren).children?.some(c => c.type === 'text' && String((c as Text).data || '').trim().length > 0);
