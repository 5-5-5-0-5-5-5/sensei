...
export async function processarDiagnostico(opts: OpcoesProcessamentoDiagnostico): Promise<ResultadoProcessamentoDiagnostico> {
  ...
  try {
    ...
    if (fastMode) {
      // Se houver convenções específicas para fastMode no config, tenta aplicá-las (mesmo que simplificado)
      const fmConfig = (config as unknown as Record<string, unknown>).fastMode as Record<string, unknown>;
      if (fmConfig?.conventions && typeof fmConfig.conventions === 'object') {
        Object.assign(config.conventions, fmConfig.conventions);
      }

      // Heurística robusta: prioriza arquivos em src/ e ignora testes/configs/docs usando convenções
      fileEntriesComAst = fileEntriesComAst.filter(fe => {
        const rel = fe.relPath || fe.fullCaminho || '';
        if (!rel) return false;

        // Deve estar em src (ou equivalente como codigo/source)
        const isSrc = isInsideSrc(rel);
        // Não deve ser teste ou configuração
        const isTest = isInsideTestsDirectory(rel);
        const isConfig = isInsideConfigDirectory(rel);
